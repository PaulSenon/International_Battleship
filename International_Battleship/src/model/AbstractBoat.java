...
public abstract class AbstractBoat implements BoatInterface {
    ...
    private List<CoordInfos> coords;
    ...
    private List<Boolean> stateOfFragment;
     ...
     */
    public List<CoordInfos> getCoords() {
     ...
     */
    public List<CoordInfos> getCoordsForDirection(Direction direction){
        List<CoordInfos> coords = new ArrayList<>();
        ...
        switch (direction){
            case EAST:
                ...
                for (int i = start; i<=stop; i++) {
                    coords.add(new CoordInfos(i,this.pivot.getY()));
                }
                break;
            case WEST:
                start = this.pivot.getX() + backParts;
            ...
            case SOUTH:
                ...
                for (int i = start; i<=stop; i++) {
                    coords.add(new CoordInfos(this.pivot.getX(), i));
                }
                break;
            case NORTH:
                start = this.pivot.getY() + backParts;
...
//	}
    ...
    /**
     * Showot the fragment of the boat
     * @param target
     * @return the result
     */
    public ResultShoot shoot(Coord target) {
        List<CoordInfos> coords = this.getCoords();
        ResultShoot result = ResultShoot.DESTROYED;
        for (CoordInfos coord : coords){
            if (coord.equals(target)){
                //Check if the target is already destroyed
                if (coord.getState()) {
                    return ResultShoot.ALREADY_TOUCHED;
                }
                    coord.destroy();
            }
            //Check if the boat is destroyed or just touched
            if (!coord.getState()){
